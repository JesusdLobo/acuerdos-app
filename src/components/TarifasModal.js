...
function TarifasModal({ show, onHide, acuerdo }) {
  ...
  const fetchAssignedTarifas = async () => {
    try {
      const resp = await axios.get(`${BASE_URL}/Acuerdos/${acuerdo.idAcuerdo}/tarifas`);
      setAssignedTarifas(resp.data || []);
    } catch (error) {
      console.error('Error al obtener tarifas asignadas:', error);
      setAssignedTarifas([]);
    }
  };

  const fetchAllTarifas = async () => {
    try {
  ...
  const isTarifaAssigned = (tarifaId) => 
    assignedTarifas.some((t) => t.idTarifa === tarifaId);

  
  const handleTarifaCheck = (tarifa, checked) => {
    if (checked) {
 
      setAssignedTarifas((prev) => [
        ...prev,
        {
          idTarifaAcuerdo: 0,
      ...
      // Quitar
      setAssignedTarifas((prev) => 
        prev.filter((t) => t.idTarifa !== tarifa.idTarifa)
      );
    }
  };

  const handleRemoveAssigned = (tarifaId) => {
    setAssignedTarifas((prev) => prev.filter((t) => t.idTarifa !== tarifaId));
  };
  ...
  const handleSave = async () => {
    ...
    const payload = assignedTarifas.map((t) => ({
      idTarifaAcuerdo: t.idTarifaAcuerdo,
      ...
      {acuerdo && (
        <>
          ...
          <Modal.Body>
            ...
            <div className="row">
              {/* Columna: Tarifas Disponibles */}
              <div className="col-6">
                <h5>Tarifas Disponibles</h5>
                {/* Filtro para las disponibles */}
              ...
              {/* Columna: Tarifas Asignadas */}
              <div className="col-6">
                <h5>Tarifas Asignadas: {assignedTarifas.length}</h5>
                {/* Filtro para las asignadas */}
                <Form.Group className="mb-2">
                  <Form.Control
                    type="text"
                    placeholder="Filtrar por nombre..."
